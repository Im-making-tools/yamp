...
def get_installation(binary) -> JavaRuntime:
    ...
    if os.name == 'nt':
        full_ver_str = subprocess.getoutput([binary, '-version']).split('\n')
    else:
        ...
        async def _worker(session):
            ...
            while not task_queue.empty():
                ...
                if 'type' not in data or check_update or not version_match:
                    ...
                    except RuntimeError:  # Exception with the coroute
                        self.log.error(f"Fetching [gray50]{rid}[/gray50] raised internal error. Opt={opt}")
                        error = True
